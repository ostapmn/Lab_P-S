...
naiveBayes <- setRefClass("naiveBayes",
       ...
       methods = list(
                    ...
                    fit = function(X, y)
                    {
                      ...
                      #Z <- stats::model.matrix(~ y - 1)              # docs × classes (one-hot)
                      #C <- t(Z) %*% Xmat                             # K × V
                      #rownames(C) <- classes; colnames(C) <- vocab
                      ...
                      if (isTRUE(fitted)){
                        ...
                        if (length(terms) == 0) {
                          ...
                          return(list(class = pred, posterior = scores / sum(scores),
                                      used_terms = character(0), used_counts = integer(0)))
                        }
...
model = naiveBayes()
model$fit()
model$predict("I became insane, with long intervals of horrible horrible sanity.")
